"""
Database = dictionary
keys -> ids for patients
value -> dictionary

{ 1: {"id": 1, "name": "Rebecca", "blood_type": "O+"}
2: {"id": 1, "name": "Rebecca", "blood_type": "O+"}
3: {"id": 1, "name": "Rebecca", "blood_type": "O+"} }
"""
...
def validate_input_data(in_data):
    ...
    for key, value_type in zip(expected_keys, expected_types):
        ...
        if type(in_data[key]) is not value_type:
            return "Key {} has incorrect value type".format(key)
    return True
...
def validate_input_data_add_tests(in_data):
    if type(in_data) is not dict:
        return "Input is not a dictionary"
...
def add_test_driver(in_data):
    validation = validate_input_data_add_tests(in_data)
    if validation is not True:
        return validation, 400
